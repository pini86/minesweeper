...
import { MenuItem, MenuListItem } from "../window/Window";
import {
  MenuGroups,
  MenuItems,
  GameItems,
...
import type { MouseEventType, Dispatch } from "../../interfaces/Interfaces";

const separators = new Set(["NEW"]);

function handleMenuListItemClick(
  group: string,
  item: string,
  dispatch: Dispatch
) {
    ...
    switch (item) {
      ...
      case GameItems.BEGINNER:
        dispatch({
          ...
          state: { ...DEFAULT_STATE, rows: 8, columns: 8, bombs: 10 },
        });
        return;
      case GameItems.INTERMEDIATE:
        dispatch({
          type: ActionTypes.RESET_GAME,
          state: { ...DEFAULT_STATE, rows: 16, columns: 16, bombs: 40 },
        });
        return;
      case GameItems.EXPERT:
        dispatch({
          type: ActionTypes.RESET_GAME,
          state: { ...DEFAULT_STATE, rows: 16, columns: 30, bombs: 99 },
        });
        return;
      default:
...
export default function Menu({ dispatch }: MenuProps) {
